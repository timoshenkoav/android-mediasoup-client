...
namespace mediasoupclient {
    ...
    SendTransportListenerJni::OnConnect(Transport * /*transport*/, const json &dtlsParameters) {
        ...
        Java_Listener_onConnect(
                env, j_listener_, j_transport_,
                webrtc::NativeToJavaString(env, dtlsParameters.dump()));
    ...
    void SendTransportListenerJni::OnConnectionStateChange(
        ...
        Java_Listener_onConnectionStateChange(
                env, j_listener_, j_transport_, webrtc::NativeToJavaString(env, connectionState));
    }

    std::future<std::string> SendTransportListenerJni::OnProduce(
            SendTransport * /*transport*/, const std::string &kind, json rtpParameters,
            const json &appData) {
        MSC_TRACE();
        JNIEnv *env = webrtc::AttachCurrentThreadIfNeeded();
        auto result = Java_Listener_onProduce(
                ...
                j_listener_,
                j_transport_,
                webrtc::NativeToJavaString(env, kind),
                webrtc::NativeToJavaString(env, rtpParameters.dump()),
                webrtc::NativeToJavaString(env, appData.dump()));
        std::promise<std::string> promise;
        promise.set_value(mediasoupclient::JavaToNativeString(env, result));
        return promise.get_future();
    }
    ...
    std::future<std::string> SendTransportListenerJni::OnProduceData(
        ...
        auto result = Java_Listener_onProduceData(
                ...
                j_listener_,
                j_transport_,
                webrtc::NativeToJavaString(env, label),
                webrtc::NativeToJavaString(env, protocol),
                webrtc::NativeToJavaString(env, sctpStreamParameters.dump()),
                webrtc::NativeToJavaString(env, appData.dump()));
        std::promise<std::string> promise;
        promise.set_value(mediasoupclient::JavaToNativeString(env, result));
        return promise.get_future();
    ...
    Transport *ExtractNativeTransport(JNIEnv *env, const webrtc::JavaRef<jobject> &j_transport) {
        auto *pc = reinterpret_cast<Transport *>(Java_Transport_getNativeTransport(env,
                                                                                   j_transport));
    ...
    static webrtc::ScopedJavaLocalRef<jstring> JNI_Transport_GetId(
            JNIEnv *env, const webrtc::JavaParamRef<jobject> &j_transport) {
        MSC_TRACE();

        auto id = ExtractNativeTransport(env, j_transport)->GetId();
        return webrtc::NativeToJavaString(env, id);
    }
    ...
    static webrtc::ScopedJavaLocalRef<jstring> JNI_Transport_GetConnectionState(
        ...
        return webrtc::NativeToJavaString(env, state);
    }
    ...
    static webrtc::ScopedJavaLocalRef<jstring> JNI_Transport_GetAppData(
        ...
        return webrtc::NativeToJavaString(env, appData);
    }
    ...
    static jboolean
    JNI_Transport_IsClosed(JNIEnv *env, const webrtc::JavaParamRef<jobject> &j_transport) {
        MSC_TRACE();
    ...
    static void JNI_Transport_RestartIce(
        ...
        try {
            ...
            if (!j_iceParameters.is_null()) {
                iceParameters = json::parse(webrtc::JavaToNativeString(env, j_iceParameters));
            }
    ...
    static void JNI_Transport_UpdateIceServers(
            JNIEnv *env, const webrtc::JavaParamRef<jobject> &j_transport,
            const webrtc::JavaParamRef<jstring> &j_iceServers) {
        MSC_TRACE();

        try {
            auto iceServers = json::array();
            if (!j_iceServers.is_null()) {
                iceServers = json::parse(mediasoupclient::JavaToNativeString(env, j_iceServers));
            }
    ...
    static void JNI_Transport_Close(JNIEnv *env, const webrtc::JavaParamRef<jobject> &j_transport) {
        MSC_TRACE();
    ...
    static jlong JNI_SendTransport_GetNativeTransport(JNIEnv *env, jlong j_transport) {
        ...
        return webrtc::NativeToJavaPointer(
                reinterpret_cast<OwnedSendTransport *>(j_transport)->transport());
    }
    ...
    static webrtc::ScopedJavaLocalRef<jobject> JNI_SendTransport_Produce(
            ...
            jlong j_track,
            const webrtc::JavaParamRef<jobjectArray> &j_encodings,
            const webrtc::JavaParamRef<jstring> &j_codecOptions,
        ...
        try {
            ...
            if (!j_codecOptions.is_null()) {
                codecOptions = json::parse(
                        mediasoupclient::JavaToNativeString(env, j_codecOptions));
            }
            ...
            auto producer = transport->Produce(listener, track, &encodings, &codecOptions, NULL,
                                               appData);
    ...
    static jlong JNI_RecvTransport_GetNativeTransport(JNIEnv *env, jlong j_transport) {
        ...
        return webrtc::NativeToJavaPointer(
                reinterpret_cast<OwnedRecvTransport *>(j_transport)->transport());
    }
    ...
    static webrtc::ScopedJavaLocalRef<jobject> JNI_RecvTransport_Consume(
            ...
            const webrtc::JavaParamRef<jstring> &j_producerId,
            const webrtc::JavaParamRef<jstring> &j_kind,
            const webrtc::JavaParamRef<jstring> &j_rtpParameters,
            const webrtc::JavaParamRef<jstring> &j_appData) {
        MSC_TRACE();

        try {
            auto listener = new ConsumerListenerJni(env, j_listener);
            auto id = webrtc::JavaToNativeString(env, j_id);
            auto producerId = webrtc::JavaToNativeString(env, j_producerId);
            auto kind = webrtc::JavaToNativeString(env, j_kind);
            auto rtpParameters = json::object();
            if (!j_rtpParameters.is_null()) {
                rtpParameters = json::parse(webrtc::JavaToNativeString(env, j_rtpParameters));
            }
            ...
            auto consumer = transport->Consume(listener, id, producerId, kind, &rtpParameters,
                                               appData);
    ...
    webrtc::ScopedJavaLocalRef<jobject> NativeToJavaSendTransport(
        ...
        auto j_transport =
                Java_SendTransport_Constructor(env,
                                               webrtc::NativeToJavaPointer(ownedSendTransport));
    ...
    webrtc::ScopedJavaLocalRef<jobject> NativeToJavaRecvTransport(
        ...
        auto j_transport =
                Java_RecvTransport_Constructor(env,
                                               webrtc::NativeToJavaPointer(ownedRecvTransport));
        listener->SetJTransport(env, j_transport);
        return webrtc::ScopedJavaLocalRef<jobject>(env, j_transport.Release());
    }
